...
form.addEventListener('submit',function(e){
  e.preventDefault();// as we want to take input so voiding deafult action of form
  const height=parseInt(document.querySelector('#height').value);
  const weight=parseInt(document.querySelector('#weight').value);
  console.log(height);
  ...
  else
  {
    ...
    else if(bmi>=18.8&&bmi<24.9)
    {
      ...
      result.innerHTML+='<br/>Over weight';
    }
  }
...
function endGame() {
  userInput.value = '';
  userInput.setAttribute('disabled', '');//will disable the form sumbit button
  //setAttribute require key,value pair (if this attribute has no value --> so value is empty string)
  //setAttribute('font-color','red')
  p.classList.add('button');//classList give list of all classes it is in(if it have mutliple classes)
  //p.className='button'  --> it will overwrite
